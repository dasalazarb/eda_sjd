...
def _parse_args() -> MergeConfig:
    ...
    parser.add_argument(
        "--output-base",
        type=Path,
        default=ANALYTIC_DIR / "visits_long_collapsed_by_interval_codebook_corrected_ans_merged",
...
def run(config: MergeConfig) -> None:
    ...
    print_script_overview(
        "17_merge_ans_autonomic_columns.py",
        "Merge ans/autonomic columns by suffix and export both parquet+csv.",
    )

    print_step(1, "Load input")
    df = _load_table(config.input_path)

    print_step(2, "Merge ans/autonomic columns")
    merged, merged_pairs = _merge_ans_autonomic_columns(df)
